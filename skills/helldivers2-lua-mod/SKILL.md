...
独立冲锋枪模式的当前资源对照、下挂骨骼证据与尚缺的父子关联见 [DBS-2 模式研究](references/DBS2_MODE.md)。

双武器模式（2026-09-26 研究中，未实现）：仲裁者/司炉者本版 `WeaponData` 记录左功能字 11、右功能字 3，而 DBS-2 为 3/0；旧版司炉者左功能枚举曾是 12，不能把历史值拷入当前内存。DBS-2 使用 `WeaponRounds` 单独两发、司炉者的主枪与下挂各用不同 `WeaponMagazine`；把 DBS-2 单发/双管切成自动只会复用霰弹弹池，不能凭此产生独立 15 发冲锋枪。用 Filediver 定向提取当前 `.unit`、`.state_machine`：DBS-2 和司炉者都具 `attach_underbarrel` 骨骼，但前者挂在 `barrel_pivot`、后者在 `boss`；插槽不是挂载/网络同步功能的证明。需先逆出本版主枪→下挂子实体和弹药绑定、动画、切换控制，再做新弹丸/弹匣；禁止只写 +184 或复制旧版枚举就声称完成。
